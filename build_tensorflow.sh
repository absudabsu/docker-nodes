...
apt install -y pkg-config g++ zlib1g-dev unzip python
...
apt update
apt install -y libcupti-dev
git clone --branch r1.7 https://github.com/tensorflow/tensorflow 
cd tensorflow
...
export TF_NEED_OPENCL=1 # use a bunch of different cpus and gpus as backend
...
pip install /tmp/tensorflow_pkg/tensorflow-1.7.0rc1-cp35-cp35m-linux_x86_64.whl
